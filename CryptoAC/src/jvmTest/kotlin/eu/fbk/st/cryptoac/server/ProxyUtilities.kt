    ...
    companion object {
        ...
        fun addUserInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.post<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}users/${core}/") {
       ...
       fun deleteUserInRBAC_MOCK(
           ...
           login: Boolean = true,
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.delete<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}users/${core}/${username}")
                    assertEquals(expectedStatus, response.status)
                    myJson.decodeFromString<OutcomeCode>(response.readText()).apply {
                        assertEquals(expectedCode, this)
                    }
        ...
        fun addRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.post<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}roles/${core}/") {
                        contentType(ContentType.Application.FormUrlEncoded)
        ...
        fun deleteRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.delete<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}roles/${core}/${roleName}")
                    assertEquals(expectedStatus, response.status)
                    myJson.decodeFromString<OutcomeCode>(response.readText()).apply {
                        assertEquals(expectedCode, this)
                    }
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    ...
                    val response = it.submitFormWithBinaryData<HttpResponse>(
                        url = "$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/",
                        formData = formData {
        ...
        fun addFileFormUrlEncodedInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.post<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/") {
                        contentType(ContentType.Application.FormUrlEncoded)
        ...
        fun deleteFileInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.delete<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/${fileName}")
                    assertEquals(expectedStatus, response.status)
                    myJson.decodeFromString<OutcomeCode>(response.readText()).apply {
                        assertEquals(expectedCode, this)
                    }
        ...
        fun readFileInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/${fileName}")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
        ...
        fun writeFileFormUrlEncodedInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.patch<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/") {
                        contentType(ContentType.Application.FormUrlEncoded)
        ...
        fun assignUserToRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.post<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}assignments/${core}/") {
                        contentType(ContentType.Application.FormUrlEncoded)
        ...
        fun revokeUserFromRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.delete<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}assignments/${core}/${username}/${roleName}")
                    assertEquals(expectedStatus, response.status)
                    myJson.decodeFromString<OutcomeCode>(response.readText()).apply {
                        assertEquals(expectedCode, this)
                    }
        ...
        fun assignPermissionToRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.post<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}permissions/${core}/") {
                        contentType(ContentType.Application.FormUrlEncoded)
        ...
        fun revokePermissionFromRoleInRBAC_MOCK(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.delete<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}permissions/${core}/${roleName}/${fileName}/${permission}")
                    assertEquals(expectedStatus, response.status)
                    myJson.decodeFromString<OutcomeCode>(response.readText()).apply {
                        assertEquals(expectedCode, this)
                    }
        ...
        fun getUsers(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}users/${core}/")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
        ...
        fun getRoles(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}roles/${core}/")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
        ...
        fun getFiles(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}files/${core}/")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
        ...
        fun getAssignments(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}assignments/${core}/")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
        ...
        fun getPermissions(
            ...
            runBlocking {
                TestUtilities.getKtorClientJetty().use {
                    if (login) loginProxy(it, loggedUser)
                    val response = it.get<HttpResponse>("$HTTPS$proxyBaseAPI${API.CRYPTOAC}permissions/${core}/")
                    assertEquals(expectedStatus, response.status)
                    if (expectedCode == OutcomeCode.CODE_000_SUCCESS) {
