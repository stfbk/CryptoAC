...
internal class DMInterfaceCloudTest : DMInterfaceTest() {

    override val dm: DMInterface =
        DMInterfaceCloud(Parameters.dmInterfaceCloudParameters)
    ...
    override fun `configure with wrong parameters fails`() {
        ...
        /** configure with wrong parameters */
        run {
            val wrongParametersClass = adminCoreRBACMQTTParameters
            assertThrows<IllegalArgumentException> {
                dm.configure(wrongParametersClass)
            }

            val wrongParametersValues = CoreParametersCLOUD(
                User(
                    name = Constants.ADMIN,
                    isAdmin = true,
                ),
                cryptoType = Parameters.cryptoType,
                mmMySQLInterfaceParameters = Parameters.MMInterfaceMySQLParameters,
                rmCloudInterfaceParameters = Parameters.rmInterfaceCloudParameters,
            ...
            assert(dm.configure(wrongParametersValues) == OutcomeCode.CODE_018_INTERFACE_CONFIGURATION_PARAMETERS)
        }
    }
