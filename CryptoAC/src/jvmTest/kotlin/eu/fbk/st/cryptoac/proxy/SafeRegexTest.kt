...
internal class SafeRegexTest {
    ...
    fun `valid texts validate against regex`() {
        assert(SafeRegex.TEXT.matches(exampleSafeTextNoSpacesRegex))
        assert(SafeRegex.URL.matches(exampleUrlRegex))
        assert(SafeRegex.URI.matches(exampleUriRegex))
        assert(SafeRegex.IPV4.matches(exampleIpv4Regex))
        assert(SafeRegex.URL_OR_IPV4.matches(exampleUrlRegex))
        assert(SafeRegex.URL_OR_IPV4.matches(exampleIpv4Regex))
        assert(SafeRegex.BASE64.matches(exampleBase64Regex))
        assert(SafeRegex.AWS_ACCESS_KEY.matches(exampleAwsAccessKeyRegex))
        assert(SafeRegex.AWS_SECRET_KEY.matches(exampleAwsSecretKeyRegex))
    }
