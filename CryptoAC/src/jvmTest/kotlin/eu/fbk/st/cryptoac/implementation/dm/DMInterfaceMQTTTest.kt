...
internal class DMInterfaceMQTTTest : DMInterfaceTest(), MqttCallback {
    ...
    override fun setUpAll() {
        ...
        val brokerBaseAPI = "tcp://${Parameters.dmInterfaceMQTTParameters.url}:${Parameters.dmInterfaceMQTTParameters.port}"
        val persistence = MemoryPersistence()
        client = CryptoACMqttClient(brokerBaseAPI, generateRandomClientId(), persistence)
        client!!.setCallback(this)
        dm = DMInterfaceMQTT(Parameters.dmInterfaceMQTTParameters, client!!)
    }
    ...
    @Test
    override fun `configure with wrong parameters fails`() {
        /** No configuration needed for DM MQTT */
    }
    ...
    fun `create a client once works`() {
        ...
        runBlocking {
            val command = (dm as DMInterfaceMQTT).getCreateClientCommand(
                username,
                "passwordUserTest1"
            )
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(command)) ==
        ...
        val clearCommand = (dm as DMInterfaceMQTT).getDeleteClientCommand(username)
        assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(clearCommand)) ==
                OutcomeCode.CODE_000_SUCCESS
        )
        topicsAndMessages[dynsecTopicResponse]!!.clear()
    }
    ...
    fun `delete a client twice or with blank username fail`() {
        ...
        runBlocking {
            val clearCommand = (dm as DMInterfaceMQTT).getDeleteClientCommand("")
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(clearCommand)) ==
                    OutcomeCode.CODE_019_MISSING_PARAMETERS)
        }
    ...
    fun `create a role once works`() {
        ...
        /** create a role once */
        runBlocking {
            val command = (dm as DMInterfaceMQTT).getCreateRoleCommand(
                roleName,
            )
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(command)) ==
                    OutcomeCode.CODE_000_SUCCESS)
        ...
        client!!.unsubscribe(dynsecTopicResponse)
        val clearCommand = (dm as DMInterfaceMQTT).getDeleteRoleCommand(roleName)
        assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(clearCommand)) ==
                OutcomeCode.CODE_000_SUCCESS
        )
        topicsAndMessages[dynsecTopicResponse]!!.clear()
    ...
    fun `delete a role twice or with blank role name fail`() {
        ...
        runBlocking {
            val clearCommand = (dm as DMInterfaceMQTT).getDeleteRoleCommand("")
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(clearCommand)) ==
                    OutcomeCode.CODE_019_MISSING_PARAMETERS)
        }
    ...
    fun `assign a client to a role once works`() {
        ...
        runBlocking {
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getCreateClientCommand(
                    username,
                    "passwordUserTest1"
                )
            )) == OutcomeCode.CODE_000_SUCCESS)
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getCreateRoleCommand(
                    roleName,
                )
            )) == OutcomeCode.CODE_000_SUCCESS)
            ...
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getAddClientRoleCommand(
                    username,
                    roleName,
                )
            )) == OutcomeCode.CODE_000_SUCCESS)
        ...
        client!!.unsubscribe(dynsecTopicResponse)
        assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
            (dm as DMInterfaceMQTT).getDeleteClientCommand(username)
        )) == OutcomeCode.CODE_000_SUCCESS)
        assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
            (dm as DMInterfaceMQTT).getDeleteRoleCommand(roleName)
        )) == OutcomeCode.CODE_000_SUCCESS)
        topicsAndMessages[dynsecTopicResponse]!!.clear()
    }
    ...
    fun `remove a client from a role once works`() {
        ...
        runBlocking {
            ...
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getRemoveClientRoleCommand(
                    username,
                    roleName,
                )
            )) == OutcomeCode.CODE_000_SUCCESS)
    ...
    fun `remove a client from a role twice or non-existing client from role or non-existing role or blank name fail`() {
        ...
        runBlocking {
            assert(
                (dm as DMInterfaceMQTT).sendDynsecCommand(
                    hashSetOf(
                        (dm as DMInterfaceMQTT).getCreateClientCommand(
                            username,
                            "passwordUserTest1"
    ...
    fun `assign a role to a topic once works`() {
        ...
        runBlocking {
            ...
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getAddRoleACLCommand(
                    roleName,
    ...
    fun `remove a role from a topic once works`() {
        ...
        runBlocking {
            ...
            assert((dm as DMInterfaceMQTT).sendDynsecCommand(hashSetOf(
                (dm as DMInterfaceMQTT).getRemoveRoleACLCommand(
                    roleName,
                    AclType.publishClientSend,
                    topicName,
                )
