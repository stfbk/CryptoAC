...
external interface MQTTContentProps : RBACContentProps {
    var topics: MutableList<String>
}

external interface MQTTContentState : RBACContentState {
...
/**
 * The MQTTContent React component, consisting of:
 ...
 */
class MQTTContent: RBACContent<MQTTContentProps, MQTTContentState>() {
     ...
     */
    override fun MQTTContentState.init() {
        logger.info { "Initializing the state of the MQTTContent component" }
        users = listOf()
        roles = listOf()
        files = listOf()
        assignments = listOf()
        ...
        MainScope().launch {
            ...
            while(true) {
                ...
                val mqttMessage = Json.decodeFromString<MQTTMessage>(serializedMQTTMessage)
...
fun rbacMQTTContent(handler: MQTTContentProps.() -> Unit): ReactElement {
    return createElement {
        child(MQTTContent::class) {
            this.attrs(handler)
        }
    }!!
}
