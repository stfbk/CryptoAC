...
external interface RBACContentProps : Props {
...
external interface RBACContentState : State {
    /** The list of users */
...
abstract class RBACContent<P : RBACContentProps, S : RBACContentState> : RComponent<P, S>() {
        ...
        return if (httpResponse != null) {
            if (httpResponse.status == HttpStatusCode.OK) {
                when (type) {
                    ...
                    ElementType.FILE -> {
                        val files: HashSet<eu.fbk.st.cryptoac.core.elements.File> = Json.decodeFromString(httpResponse.readText())
