...
import eu.fbk.st.cryptoac.view.components.custom.CryptoACEditProfileFormProps
...
suspend fun getAssignments(
    ...
    val actualEndpoint =
        "$baseURL${API.CRYPTOAC}${API.ASSIGNMENTS.replace("{Core}", props.coreTypeProp.toString())}?" +
                (if (username != null) "${SERVER.USERNAME}=$username" else "") +
                if (roleName != null) "${SERVER.ROLE_NAME}=$roleName" else ""
    return getElements(
        props = props,
        endpoint = actualEndpoint,
        errorCode = OutcomeCode.CODE_007_ROLETUPLE_NOT_FOUND,
        type = RBACElementType.ROLETUPLE
    )
}
...
suspend fun getPermissions(
    ...
    val actualEndpoint =
        "$baseURL${API.CRYPTOAC}${API.PERMISSIONS.replace("{Core}", props.coreTypeProp.toString())}?" +
                (if (roleName != null) "${SERVER.ROLE_NAME}=$roleName" else "") +
                if (resourceName != null) "${SERVER.RESOURCE_NAME}=$resourceName" else ""
    return getElements(
        props = props,
        endpoint = actualEndpoint,
        errorCode = OutcomeCode.CODE_008_PERMISSIONTUPLE_NOT_FOUND,
        type = RBACElementType.PERMISSIONTUPLE
    )
}
