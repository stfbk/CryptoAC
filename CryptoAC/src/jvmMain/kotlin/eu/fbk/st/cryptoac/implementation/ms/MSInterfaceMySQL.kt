 ...
 * as a MySQL8+ database configured with the given [msMySQLInterfaceParameters].
 ...
 */
class MSInterfaceMySQL(private val msMySQLInterfaceParameters: MSInterfaceMySQLParameters) : MSInterface() {
    ...
    private val jDBUrl: String = "jdbc:mysql://${msMySQLInterfaceParameters.url}:${msMySQLInterfaceParameters.port}"

    init {
        connectionProperties.setProperty("user", msMySQLInterfaceParameters.username)
        connectionProperties.setProperty("password", msMySQLInterfaceParameters.password)
    ...
    override fun initAdmin(admin: User, adminRoleTuple: RoleTuple): OutcomeCode {
        ...
        val sqlFile = MSInterfaceMySQL::class.java.getResourceAsStream(INIT_SQL_CODE)
        if (sqlFile == null) {
        ...
        val asymEncPublicKeyEncoded = Base64.getEncoder().encodeToString(admin.asymEncKeys!!.public)
        val asymSigPublicKeyEncoded = Base64.getEncoder().encodeToString(admin.asymSigKeys!!.public)
    ...
    override fun initUser(user: User): OutcomeCode {
        ...
        val asymEncPublicKeyEncoded = Base64.getEncoder().encodeToString(user.asymEncKeys!!.public)
        val asymSigPublicKeyEncoded = Base64.getEncoder().encodeToString(user.asymSigKeys!!.public)
        val userValues = LinkedHashMap<String, Any?>()
            .apply { put(asymEncPublicKeyColumn, asymEncPublicKeyEncoded) }
            .apply { put(asymSigPublicKeyColumn, asymSigPublicKeyEncoded) }
     ...
     * with user's MS configuration parameters.
     *
     * In this implementation, also create an account in the database for the [newUser].
     */
    override fun addUser(newUser: User): WrapperMSParameters {
        ...
        if (getUsers(username = newUser.name, status = ElementStatus.DELETED).isNotEmpty()) {
            ...
            return WrapperMSParameters(OutcomeCode.CODE_013_USER_WAS_DELETED)
        }

        /** TODO check password generation. */
        val passwordBytes = ByteArray(20)
        Random().nextBytes(passwordBytes)
        val newPassword = Base64.getEncoder().encodeToString(passwordBytes)
        ...
        createInsertStatement(usersTable, ArrayList<ArrayList<Any?>>().apply { add(adminUserValues) }).use {
            if (it.executeUpdate() != 1) {
                ...
                return WrapperMSParameters(OutcomeCode.CODE_001_USER_ALREADY_EXISTS)
            }
        }
        ...
        return WrapperMSParameters(
            parameters = MSInterfaceMySQLParameters(
                username = newUser.name, password = newPassword,
                port = msMySQLInterfaceParameters.port, url = msMySQLInterfaceParameters.url,
            )
        )
    }
    ...
    override fun addRole(newRole: Role): OutcomeCode {
        ...
        val roleValues = ArrayList<Any?>()
            ...
            .apply { add(Base64.getEncoder().encodeToString(newRole.asymEncKeys!!.public)) }
            .apply { add(Base64.getEncoder().encodeToString(newRole.asymSigKeys!!.public)) }
     ...
     * [fileToken] and having the specified [status], if given, starting
     * from the [offset] limiting the number of files to return to the
     * given [limit].
     */
    override fun getFiles(
        ...
        createSelectStatement(
            table = if (status == ElementStatus.DELETED) deletedFilesTable else filesTable,
            whereParameters = whereParameters,
            limit = limit, offset = offset,
        ).use {
            val rs = it.executeQuery()
            ...
            while (rs.next()) {
                val roleTuple = RoleTuple(
                    ...
                    encryptedAsymEncKeys = EncryptedAsymKeys(
                        public = Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(encryptedAsymEncPublicKeyColumn))),
                        private = Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(encryptedAsymEncPrivateKeyColumn))),
                        type = AsymKeysType.ENC,
                    ),
                    encryptedAsymSigKeys = EncryptedAsymKeys(
                        public = Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(encryptedAsymSigPublicKeyColumn))),
                        private = Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(encryptedAsymSigPrivateKeyColumn))),
                        type = AsymKeysType.SIG,
                    ),
                )
                roleTuple.updateSignature(
                    newSignature = Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(signatureColumn))),
                    newSigner = ADMIN,
            ...
            while (rs.next()) {
                val permissionTuple = PermissionTuple(
                    ...
                    encryptedSymKey = EncryptedSymKey(
                        Base64.getDecoder().decode(sanitizeStringFromHTLM(rs.getString(encryptedSymKeyColumn)))
                    )
                )
    ...
    override fun getPublicKey(
        ...
        createSelectStatement(
            ...
            val rs = it.executeQuery()
            if (rs.next()) {
                asymPublicKeyBytes = Base64.getDecoder()
                    .decode(sanitizeStringFromHTLM(rs.getString(
    ...
    override fun updateRoleAsymKeys(
        ...
        val whereParameters = LinkedHashMap<String, Any?>().apply { put(roleNameColumn, roleName) }
        val values = LinkedHashMap<String, Any?>()
            .apply { put(asymEncPublicKeyColumn, Base64.getEncoder().encodeToString(newAsymEncPublicKey.encoded)) }
            .apply { put(asymSigPublicKeyColumn, Base64.getEncoder().encodeToString(newAsymSigPublicKey.encoded)) }
    ...
    override fun updatePermissionTuple(updatedPermissionTuple: PermissionTuple): OutcomeCode {
        ...
        val values = LinkedHashMap<String, Any?>()
            ...
            .apply { put(signatureColumn, Base64.getEncoder().encodeToString(updatedPermissionTuple.signature)) }
            .apply { put(symKeyVersionNumberColumn, updatedPermissionTuple.symKeyVersionNumber) }
    ...
    private fun createArray(roleTuple: RoleTuple): ArrayList<Any?> {
        return ArrayList<Any?>()
            ...
            .apply { add(Base64.getEncoder().encodeToString(roleTuple.encryptedAsymEncKeys!!.public)) }
            .apply { add(Base64.getEncoder().encodeToString(roleTuple.encryptedAsymEncKeys!!.private)) }
            .apply { add(Base64.getEncoder().encodeToString(roleTuple.encryptedAsymSigKeys!!.public)) }
            .apply { add(Base64.getEncoder().encodeToString(roleTuple.encryptedAsymSigKeys!!.private)) }
            .apply { add(roleTuple.roleVersionNumber) }
            .apply { add(Base64.getEncoder().encodeToString(roleTuple.signature)) }
    }
    ...
    private fun createArray(permissionTuple: PermissionTuple): ArrayList<Any?> {
        return ArrayList<Any?>()
            ...
            .apply { add(Base64.getEncoder().encodeToString(permissionTuple.encryptedSymKey!!.key)) }
            ...
            .apply { add(Base64.getEncoder().encodeToString(permissionTuple.signature)) }
    }
    ...
    private fun createArray(fileTuple: FileTuple): ArrayList<Any?> {
        return ArrayList<Any?>()
            ...
            .apply { add(Base64.getEncoder().encodeToString(fileTuple.signature)) }
    }
